<?php
$pageTitle = 'Editar Cliente - FiadoApp';
...
<section class="form-card">
    ...
    <div class="form-group" style="position:relative;">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
            ...
            <button type="button" class="btn-limpar" onclick="limparCampo('sobrenome')" title="Limpar">×</button>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
            <input type="text" id="referencia" value="<?= htmlspecialchars($cliente['referencia'] ?? '') ?>" placeholder="Ex: Vizinho, Primo..." maxlength="100" style="flex:1;">
            <button type="button" class="btn-limpar" onclick="limparCampo('referencia')" title="Limpar">×</button>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
            ...
            <button type="button" class="btn-limpar" onclick="limparCampo('telefone')" title="Limpar">×</button>
        </div>
    </div>
...
.btn-limpar {
    ...
    border-radius: 6px; width: 36px; height: 36px; font-size: 18px; line-height: 1;
    cursor: pointer; flex-shrink: 0; transition: background 0.15s, color 0.15s, border-color 0.15s;
}
.btn-limpar:hover { background: var(--brand); border-color: var(--brand); color: #fff; }
</style>

<?php
$clienteIdJS = $cliente_id;
$voltaJS     = json_encode($volta);
$footerScripts = <<<SCRIPT
<script src="/assets/js/telefone.js"></script>
<script>
const CLIENTE_ID = {$clienteIdJS};
const VOLTA      = {$voltaJS};
...
async function salvarEdicao() {
    ...
    if (!nome) { showToast('O nome do cliente é obrigatório.', 'error'); return; }
    ...
    try {
        const response  = await fetch('/api/atualizar_cliente.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dados) });
        const resultado = await response.json();
        if (resultado.status === 'sucesso') {
            showToast('Cliente atualizado com sucesso!');
            setTimeout(() => { window.location.href = '/consulta.php' + (VOLTA ? '?letra=' + VOLTA : ''); }, 1500);
        } else {
            showToast(resultado.mensagem || 'Erro ao atualizar.', 'error');
        }
    } catch(e) { showToast('Erro inesperado.', 'error'); }
}
</script>
SCRIPT;
